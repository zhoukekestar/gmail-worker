...
export default async ({ req, env }) => {
  ...
  const headers = {
    ...
    Authorization: credentials?.access_token || credentials?.credentials?.access_token,
  }

  const res = fetch("https://gmail.googleapis.com/gmail/v1/users/me/messages?historyId=929386", {
    headers
  }).then(d => d.text());
  // "options": {
